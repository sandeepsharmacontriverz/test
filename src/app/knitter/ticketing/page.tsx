...
export default function page() {
  ...
  }, [searchQuery, page, limit, knitterId, isClear]);
  ...
  const fetchTicketing = async () => {
    try {
      const res = await API.get(
        `ticketing?processor=Knitter&processorId=${knitterId}&search=${searchQuery}&processSale=${checkedType}&from=${startDates ? startDates?.toISOString() : ""
        }&to=${endDates ? endDates?.toISOString() : ""
        }&status=${checkedStatus}&page=${page}&limit=${limit}&pagination=true`
      );
      if (res.success) {
        setData(res.data);
        setCount(res.count);
  ...
  const columns = [
    ...
    {
      name: <p className="text-[13px] font-medium">{translations?.ticketing?.lotNo} </p>,
    ...
    },
    {
      ...
      cell: (row: any) => (
        <Link
          href={`/knitter/ticketing/view-details?id=${row.id}`}
        >
                          ...
                          {Access?.create && (
                            <div className="search-filter-right">
                              <button
                                ...
                                onClick={() =>
                                  router.push("/knitter/ticketing/add-ticket")
                                }
                              >
                                {" "}
