...
export default function Page() {
  ...
  useEffect(() => {
    if (knitterId && formData.dataCorrectionType === "Sale") {
      fetchSales();
      fetchTicketType("sales");
    } else if (knitterId && formData.dataCorrectionType === "Process") {
      fetchProcesses();
      fetchTicketType("process");
    }
  }, [knitterId, formData.dataCorrectionType]);
  ...
  const fetchSales = async () => {
    try {
      const response = await API.get(`knitter-process?knitterId=${knitterId}`);
      if (response.success) {
  ...
  const fetchTicketType = async (type: any) => {
    try {
      const response = await API.get(
        `ticketing/escalation-type?processorType=knitter&correctionType=${type}`
      );
      if (response.success) {
          ...
          <div className="breadcrumb-inner light-bg">
            <div className="breadcrumb-left">
              <ul className="breadcrum-list-wrap">
                ...
                <li>
                  <Link href="/knitter/ticketing">{translations?.ticketing?.ticketing}</Link>
                </li>
          ...
          <div className="pt-10 w-100 d-flex justify-start customButtonGroup pb-5">
            <section>
              <button
                ...
                onClick={() => router.push("/knitter/ticketing")}
              >
