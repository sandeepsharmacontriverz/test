...
const WashingOldsales = () => {
    useTitle("Washing Oldsales");
    ...
    const fetchWashingOldSales = async () => {
        try {
            const res = await API.get(`oldsales/washing-oldsales?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
    ...
    const getDataToExport = (data: Array<any>, pagination = false) => {
        const dataToExport = data.map((element: any, index: number) => {
            return {
                ...
                "Knit/Woven/Dying Fabric type": element.fabric_type,
                "Washed fabric quantity": element.total_fabric_quantity,
    ...
    const handleExport = () => {
        if (data.length > 0) {
            ...
            exportToExcel(dataToExport, `Old washing sales data - page${page}`);
        } else {
    ...
    const handleExportAll = async () => {
        try {
            const res = await API.get(`oldsales/washing-oldsales?search=${searchQuery}&pagination=false`);
            if (res.success) {
                if (res.data.length > 0) {
                    const dataToExport = getDataToExport(res.data);
                    exportToExcel(dataToExport, "Old washing sales data");
                } else {
                    toasterError("Nothing to export!");
                }
            }
    ...
    const columns = [
        ...
        {
            name: "Knit/Woven/Dying Fabric type",
            selector: (row: any) => row.fabric_type,
            sortable: false,
        },
        {
            name: "Washed fabric quantity",
...
export default WashingOldsales;