...
const PrintingOldsales = () => {
    useTitle("Printing Oldsales");
    ...
    const fetchPrintingOldSales = async () => {
        try {
            const res = await API.get(`oldsales/printing-oldsales?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
    ...
    const handleExport = () => {
        if (data.length > 0) {
            ...
            exportToExcel(dataToExport, `Old printing sales data - page${page}`);
        } else {
    ...
    const handleExportAll = async () => {
        try {
            const res = await API.get(`oldsales/printing-oldsales?search=${searchQuery}&pagination=false`);
            if (res.success) {
                if (res.data.length > 0) {
                    const dataToExport = getDataToExport(res.data);
                    exportToExcel(dataToExport, "Old printing sales data");
                } else {
                    toasterError("Nothing to export!");
                }
            }
...
export default PrintingOldsales;