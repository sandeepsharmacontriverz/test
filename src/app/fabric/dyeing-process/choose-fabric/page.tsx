...
export default function page() {
  ...
  const getData = async () => {
    try {
      const res = await API.get(
        `fabric-process/choose-dying-fabric?fabricId=${fabricId}&programId=${programId}&weaverId=${checkedWeaverIdAlert}&knitterId=${checkedKnitIdAlert}&garmentOrderRef=${checkedGarmentOrderRef}&brandOrderRef=${checkedBrandOrderRef}&lotNo=${checkedBatchLotNo}`
      );

      if (res.success) {
        let total: number = 0;
        let data = res.data;
        const storedData: any = sessionStorage.getItem("choose-dyeing-process");
  ...
  const getLotNo = async () => {
    const url = `fabric-process/get-batch-lot?fabricId=${fabricId}&buyerType=Dyeing`;
    try {
  ...
  const getProcessors = async () => {
    const url = `fabric-process/get-processors?fabricId=${fabricId}&buyerType=Dyeing`;
    try {
      const response = await API.get(url);
      const data = response.data;
  ...
  const handleSubmit = (event: any) => {
    ...
    sessionStorage.setItem(
      "choose-dyeing-process",
      JSON.stringify([...selectedData])
    );

    router.push("/fabric/dyeing-process/new-process");
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row: any) =>
        row.knitter ? row.knitter?.name : row.weaver?.name,
      wrap: true,
      sortable: false,
    },
    {
    ...
    },
    {
      ...
      selector: (row: any) => DecimalFormat(row.total_yarn_qty),
      wrap: true,
    },
    {
    ...
    return (
      <div>
        {openFilter && (
          <div
            ref={popupRef}
            className="fixPopupFilters flex fixWidth h-full top-0 align-items-center w-auto z-10 fixed justify-center left-0 right-0 bottom-0 p-3"
          >
          ...
          <div className="breadcrumb-inner light-bg">
            <div className="breadcrumb-left">
              <ul className="breadcrum-list-wrap">
                ...
                <li>
                  <NavLink
                    href="/fabric/dyeing-process/new-process"
                    className="active"
                  >
                    New Process
                  </NavLink>
