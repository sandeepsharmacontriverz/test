...
export default function page() {
  ...
  const fetchSales = async () => {
    try {
      const response = await API.get(
        `fabric-process/dying-process?fabricId=${fabricId}&search=${code}&limit=${limit}&page=${page}&pagination=true`
      );
      if (response.success) {
  ...
  const fetchExport = async () => {
    const url = `fabric-process/export-dying-process?fabricId=${fabricId}&page=${page}&limit=${limit}&search=${code}`;
    try {
      const response = await API.get(url);
      if (response.success) {
        if (response.data) {
          handleDownload(response.data, "Dyin_process_report", ".xlsx");
        }
      }
    } catch (error) {
  ...
  const columns = [
    ...
    {
      name: <p className="text-[13px] font-medium">Dyed Fabric Quantity </p>,
      selector: (row: any) => row.fabric_quantity,
      wrap: true,
    },
    {
        ...
        <DeleteConfirmation
          ...
          onDelete={async () => {
            if (deleteItemId !== null) {
              const url = "fabric-process/delete-dying-process";
              try {
