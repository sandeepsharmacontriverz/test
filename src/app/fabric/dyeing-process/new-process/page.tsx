...
interface FormData {
  ...
  date: string;
  garmentOrderRef: string;
  brandOrderRef: string;
  buyerType: string;
  buyerId?: string | null;
  buyerFabricId?: string | null;
  processorName: string;
  processorAddress: string;
  oldFabricQuantity: number | null;
  addFabricQuantity: number | null;
  fabricQuantity: number | null;
  totalFabricQuantity: number | null;
  fabricLength: string;
  fabricGsm: any;
  fabricNetWeight: any;
  processWeight: string | null;
  weightGain: any;
  weightLoss: any;
  batchLotNo: string;
  jobDetails: string;
...
export default function page() {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    processorName: "",
    processorAddress: "",
    oldFabricQuantity: null,
    addFabricQuantity: null,
    fabricQuantity: null,
    totalFabricQuantity: null,
    fabricLength: "",
    fabricGsm: "",
    fabricNetWeight: "",
    processWeight: null,
  ...
  useEffect(() => {
    if (!roleLoading && hasAccesss?.processor?.includes("Fabric")) {
      const access = checkAccess("Fabric Dying Process");
  ...
  useEffect(() => {
    const savedData: any = sessionStorage.getItem("dyeing-process");
  ...
  useEffect(() => {
    const storedTotal = sessionStorage.getItem("choose-dyeing-process");
  ...
  const getChooseYarn = () => {
    if (formData.programId) {
      sessionStorage.setItem("dyeing-process", JSON.stringify(formData));
      router.push(
        `/fabric/dyeing-process/choose-fabric?id=${formData.programId}`
      );
    } else {
      setErrors((prev: any) => ({
        ...prev,
  ...
  const handleChange = (name?: any, value?: any, event?: any) => {
    if (name === "programId") {
      ...
      } else if (value === "Washing" || value === "Compacting") {
        setFormData((prevData: any) => ({
          ...prevData,
          [name]: value,
          buyerId: null,
      ...
      switch (name) {
        ...
        case "buyerId":
          return formData.buyerType === "Garment" &&
            ...
            : "";
        case "buyerFabricId":
          return (formData.buyerType === "Washing" ||
            formData.buyerType === "Compacting") &&
            (value?.length === 0 || value === null)
            ? "This field is required"
            : "";
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (!hasWeaversErrors) {
      try {
        ...
        const response = await API.post("fabric-process/dying-process", {
          ...formData,
          fabricId: fabricId,
          chooseFabric: chooseFabric,
          buyerId:
            formData.buyerType === "Garment" ? Number(formData?.buyerId?.value) : null,
          buyerFabricId: Number(formData?.buyerFabricId?.value),
        });
        if (response.success) {
          toasterSuccess("Process created successfully");
          sessionStorage.removeItem("dyeing-process");
          sessionStorage.removeItem("choose-dyeing-process");
          router.push(`/fabric/dyeing-process`);
        } else {
        ...
        <div className="breadcrumb-inner light-bg">
          <div className="breadcrumb-left">
            <ul className="breadcrum-list-wrap">
              ...
              <li>
                <NavLink href="/fabric/dyeing-process" className="active">
                  Process/Sale
                </NavLink>
              </li>
              <li>New Process </li>
          ...
          <div className="customFormSet">
            <div className="w-100">
              <div className="row">
                  ...
                  <>
                    <div className=" col-12 col-sm-6 mt-4 ">
                      ...
                      <Select
                        ...
                        className="dropDownFixes rounded-md formDropDown mt-1 text-sm borderBottom" options={
                  ...
                  <>
                    <div className="col-12 col-sm-6 mt-4">
                      ...
                      <Select
                        ...
                        onChange={(item: any) =>
                          handleChange("buyerFabricId", item)
                        }
                      />
                      {errors?.buyerFabricId && (
                        <div className="text-sm text-red-500">
                          {errors.buyerFabricId}
                        </div>
                      )}
                ...
                <div className=" col-12 col-sm-6 mt-4">
                  <label className="text-gray-500 text-[12px] font-medium">
                    Dying Details <span className="text-red-500">*</span>
                  </label>
                  <input
                    name="dyingDetails"
                    value={formData.dyingDetails}
                    onBlur={(e) => onBlur(e, "bill")}
                    placeholder="Dying Details"
                    onChange={(e) => handleChange("dyingDetails", e.target.value)}
                    type="text"
                    className="w-100 shadow-none h-11 rounded-md mt-1 form-control gray-placeholder text-gray-500 text-sm borderCustom"
                  />
                  {errors?.dyingDetails && (
                ...
                <div className="col-12 col-sm-6 mt-4">
                  ...
                  <input
                    name="dyingColor"
                    value={formData.dyingColor}
                    onBlur={(e) => onBlur(e, "bill")}
                    onChange={(e) => handleChange("dyingColor", e.target.value)}
                    type="text"
                    placeholder="Dyeing Color"
                    className="w-100 shadow-none h-11 rounded-md mt-1 form-control gray-placeholder text-gray-500 text-sm borderCustom"
                  />
                  {errors?.dyingColor && (
                    <div className="text-sm text-red-500">
                ...
                <div className="col-12 col-sm-6 mt-4">
                  <label className="text-gray-500 text-[12px] font-medium">
                    Finished Dyed Fabric Length in Mts{" "}
                    <span className="text-red-500">*</span>
                  </label>
                ...
                <div className="col-12 col-sm-6 mt-4">
                  <label className="text-gray-500 text-[12px] font-medium">
                    Finished Dyed Fabric Net Weight{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <input
                  ...
                  <div className="col-12 col-sm-6 mt-4">
                    ...
                    <div className="inputFile">
                      <label>
                        ...
                        <input
                          ...
                          onChange={(e) => handleChange("dyeInvoice", e?.target?.files?.[0])}
                        />
                      </label>
                    </div>
                    ...
                    {fileName.dyeInvoice && (
                      <div className="flex text-sm mt-1">
                        <GrAttachment />
                        <p className="mx-1">{fileName.dyeInvoice}</p>
                      </div>
                    )}
                    {errors?.dyeInvoice !== "" && (
                      <div className="text-sm text-red-500">
