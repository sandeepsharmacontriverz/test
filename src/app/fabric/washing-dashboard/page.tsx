...
export default function page() {
  ...
  useEffect(() => {
    if (!roleLoading && hasAccess?.processor?.includes("Fabric")) {
      const access = checkAccess("Fabric Washing Transaction");
  ...
  const fetchAlertData = async () => {
    const url = `fabric-process/washing-dashboard-all?fabricId=${fabricId}`;
    try {
      const response = await API.get(url);
  ...
  const fetchTransActionList = async () => {
    const url = `fabric-process/washing-dashboard?fabricId=${fabricId}&status=Sold&search=${code}&page=${page}&limit=${limit}&pagination=true`;
  ...
  const columnList = [
    ...
    },
    {
      name: (
        <p className="text-[13px] font-medium">
          Knitter/ Weaver/ Dying Processor Name{" "}
        </p>
      ),
      selector: (row: any) =>
        row?.weaver?.name
    ...
    // },
    {
      ...
      selector: (row: any) =>
        row?.total_yarn_qty ? row?.total_yarn_qty : row?.total_fabric_quantity,
      wrap: true,
    },
  ...
  const handleConfirmActions = async () => {
    try {
      ...
      const url = "fabric-process/update-transaction-washing";
