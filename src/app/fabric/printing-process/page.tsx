...
export default function page() {
  ...
  const fetchSales = async () => {
    try {
      const response = await API.get(
        `fabric-process/printing-process?fabricId=${fabricId}&search=${searchQuery}&limit=${limit}&page=${page}&pagination=true`
      );
      if (response.success) {
  ...
  const fetchExport = async () => {
    const url = `fabric-process/export-printing-process?fabricId=${fabricId}&page=${page}&limit=${limit}&search=${code}`;
    try {
      const response = await API.get(url);
      if (response.success) {
        if (response.data) {
          handleDownload(response.data, "Printing_Report", ".xlsx");
        }
      }
    } catch (error) {
        ...
        <DeleteConfirmation
          ...
          onDelete={async () => {
            if (deleteItemId !== null) {
              const url = "fabric-process/delete-printing-process";
              try {
