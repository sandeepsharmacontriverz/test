...
export default function page() {
  ...
  const getData = async () => {
    try {
      const res = await API.get(
        `fabric-process/choose-printing-fabric?fabricId=${fabricId}&washingId=${checkedProcessorName}&programId=${programId}&garmentOrderRef=${checkedGarmentOrderRef}&brandOrderRef=${checkedBrandOrderRef}&lotNo=${checkedBatchLotNo}`
      );
  ...
  const getProcessors = async () => {
    const url = `fabric-process/get-processors?fabricId=${fabricId}&buyerType=Printing`;
    try {
      ...
      const washingNames = data
        ...
        .map((item: any) => {
          return {
            ...item.washing,
            key: item.washing?.name,
          };
        });
      setProcessorName(washingNames);
    } catch (error) {
  ...
  const getLotNo = async () => {
    const url = `fabric-process/get-batch-lot?fabricId=${fabricId}&buyerType=Printing`;
    try {
      const response = await API.get(url);
      const data = response.data;
      setDataFilter(data);
  ...
  const handleSubmit = (event: any) => {
    ...
    router.push("/fabric/printing-process/new-process");
  };
  ...
  const handleChangeList = (
    ...
    if (name === "batchLotNo") {
      ...
      if (checkedProcessorName.includes(itemId)) {
        setCheckedProcessorName(
          checkedProcessorName.filter((item: any) => item !== itemId)
        );
      } else {
        setCheckedProcessorName([...checkedProcessorName, itemId]);
      }
                    ...
                    <div className="row">
                      ...
                      <div className="col-12 col-md-6 mt-2">
                        ...
                        <Multiselect
                          ...
                          selectedValues={dataFilter?.filter((item: any) =>
                      ...
                      <div className="col-12 col-md-6 mt-2">
                        ...
                        <Multiselect
                          ...
                          selectedValues={dataFilter?.filter((item: any) =>
                            checkedBrandOrderRef.includes(item.brand_order_ref)
                          )}
                          onKeyPressFn={function noRefCheck() { }}
                          onRemove={(selectedList: any, selectedItem: any) => {
                            handleChangeList(
          ...
          <div className="breadcrumb-inner light-bg">
            <div className="breadcrumb-left">
              <ul className="breadcrum-list-wrap">
                ...
                <li>
                  <NavLink
                    href="/fabric/printing-process/new-process"
                    className="active"
                  >
                    New Process
                  </NavLink>
