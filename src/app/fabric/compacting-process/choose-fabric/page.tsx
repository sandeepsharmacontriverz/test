...
export default function page() {
  ...
  const [checkedWashingAlert, setCheckedWashingAlert] = useState([]);
  const [checkedPrintingAlert, setCheckedPrintingAlert] = useState([]);
  ...
  const getData = async () => {
    try {
      const res = await API.get(
        `fabric-process/choose-compacting-fabric?fabricId=${fabricId}&dyingId=${checkedDyingIdAlert}&washingId=${checkedWashingAlert}&printingId=${checkedPrintingAlert}&garmentOrderRef=${checkedGarmentOrderRef}&brandOrderRef=${checkedBrandOrderRef}&lotNo=${checkedBatchLotNo}&programId=${programId}`
      );
  ...
  const getProcessors = async () => {
    const url = `fabric-process/get-processors?fabricId=${fabricId}&buyerType=Compacting`;
    try {
      ...
      const dyingWashing = dyingNames.concat(washingNames);
      const allNames = dyingWashing.concat(printingNames);
  ...
  const getLotNo = async () => {
    const url = `fabric-process/get-batch-lot?fabricId=${fabricId}&buyerType=Compacting`;
    try {
      const response = await API.get(url);
      const data = response.data;
      setDataFilter(data);
  ...
  const handleSubmit = (event: any) => {
    ...
    sessionStorage.setItem("chooseFabric", JSON.stringify([...selectedData]));
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row: any) => DecimalFormat(row.total_fabric_quantity),
      wrap: true,
    },
    {
  ...
  const clearFilterList = () => {
    ...
    setCheckedWashingAlert([]);
    setCheckedPrintingAlert([]);
                    ...
                    <div className="row">
                      <div className="col-12 col-md-6 mt-2">
                        <label className="text-gray-500 text-[12px] font-medium">
                          Select Dying/ Washing/ Printing Processor
                        </label>
                        <Multiselect
                          className="w-100 shadow-none h-11 rounded-md mt-1 form-control gray-placeholder text-gray-500 text-sm borderCustom"
                          displayValue="key"
                          selectedValues={processorName?.filter((item: any) =>
                            checkedProcessorName.includes(item.key)
                          )}
                          // Other props...
        ...
        <div className="breadcrumb-inner light-bg">
          <div className="breadcrumb-left">
            <ul className="breadcrum-list-wrap">
              ...
              <li>
                <NavLink
                  href="/fabric/compacting-process/new-process"
                  className="active"
                >
                  New Process
                </NavLink>
