...
export default function page() {
  ...
  useEffect(() => {
    if (!roleLoading && hasAccess?.processor?.includes("Fabric")) {
      const access = checkAccess("Fabric Compacting Transaction");
  ...
  const fetchAlertData = async () => {
    const url = `fabric-process/compacting-dashboard-all?fabricId=${fabricId}`;
    try {
      const response = await API.get(url);
  ...
  const fetchTransActionList = async () => {
    const url = `fabric-process/compacting-dashboard?fabricId=${fabricId}&search=${code}&page=${page}&limit=${limit}&pagination=true`;
  ...
  const columnList = [
    ...
    {
      name: <p className="text-[13px] font-medium">Garment Reference No </p>,
    ...
    try {
      ...
      const acceptedUpdateRequests = acceptedRowIds?.map((rowId) => {
        ...
        return {
          ...
          status: "Sold",
          type:
            item && item.washing_id
              ? "Washing"
              : item && item.printing_id
                ? "Printing"
                : "Dying",
        };
      });
          ...
          type:
            item && item.washing_id
              ...
              : item && item.printing_id
                ? "Printing"
                : "Dying",
          // washingId: item && item.washing_id ? item.washing_id : null,
          // printingId: item && item.printing_id ? item.printing_id : null,
          // dyingId: item && item.dying_id ? item.dying_id : null,
        };
      });
      ...
      const url = "fabric-process/update-transaction-compacting";
