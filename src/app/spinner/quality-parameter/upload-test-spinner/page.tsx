...
export default function Page() {
  ...
  const [spinnerProcess, setSpinnerProcess] = useState<any>();
  ...
  const getSpinnerProcess = async () => {
    try {
      const res = await API.get(
        `spinner-process/transaction?spinnerId=${spinnerId}&status=Sold`
      );
      if (res.success) {
        setSpinnerProcess(res.data);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  //         const matchingItem = spinnerProcess.find(
  //           (item: any) => item.lot_no === fieldValue
  //         );

  //         if (matchingItem) {
  //           updatedItem.reelLotNo = matchingItem.reel_lot_no;
  //           updatedItem.salesId = matchingItem.id;
  //           updatedItem.spinnerId = spinnerId;
  //           updatedItem.soldId = matchingItem.buyer;
  //         }
  //         return updatedItem;
  //       } else {
  //         return item;
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (!hasErrors) {
      ...
      try {
        ...
        if (res.success) {
          setIsSelected(false);
          router.push("/spinner/quality-parameter/cotton-quality-parameter");
          toasterSuccess("Test Report uploaded", 3000, filteredData.salesId);
        }
      } catch (error) {
        setIsSelected(false);
        console.log(error);
  ...
  const columns = [
    ...
    {
      name: translations.ginnerInterface.baleLotNo,
      sortable: false,
      width: "160px",
      center: true,
      cell: (row: any, index: any) => (
        <div className="py-2">
          ...
          >
            ...
            {spinnerProcess?.map((spinnerProcess: any) => (
              <option key={spinnerProcess.id} value={spinnerProcess.lot_no}>
                {spinnerProcess.lot_no}
              </option>
            ))}
          </Form.Select>
      ...
      cell: (row: any, index: any) => (
        <div className="py-2">
          <DatePicker
            ...
            className="w-full z-[999] shadow-none h-8 rounded-md mt-1 form-control gray-placeholder bg-white text-sm borderCustom"
          />
