...
export default function Page() {
  ...
  useEffect(() => {
    if (weaverId && formData.dataCorrectionType === "Sale") {
      fetchSales();
      fetchTicketType("sales");
    } else if (weaverId && formData.dataCorrectionType === "Process") {
      fetchProcesses();
      fetchTicketType("process");
    }
  }, [weaverId, formData.dataCorrectionType]);

  const fetchProcesses = async () => {
    try {
      const res = await API.get(`weaver-process/process?weaverId=${weaverId}`);
      if (res.success) {
  ...
  const fetchSales = async () => {
    try {
      const response = await API.get(`weaver-process?weaverId=${weaverId}`);
      if (response.success) {
  ...
  const fetchTicketType = async (type: any) => {
    try {
      const response = await API.get(
        `ticketing/escalation-type?processorType=weaver&correctionType=${type}`
      );
      if (response.success) {
          ...
          <div className="breadcrumb-inner light-bg">
            <div className="breadcrumb-left">
              <ul className="breadcrum-list-wrap">
                ...
                <li>
                  <Link href="/weaver/ticketing">Ticketing</Link>
                </li>
                ...
                <div className="row">
                  ...
                  <div className="col-sm-12 col-lg-6 col-md-6 mt-4">
                    ...
                    <Select
                      ...
                      menuShouldScrollIntoView={false}
                      isClearable
                      placeholder="Select Finished Batch/Lot No"
                      className="dropDownFixes rounded-md formDropDown mt-1 text-sm borderBottom"
          ...
          <div className="pt-10 w-100 d-flex justify-start customButtonGroup pb-5">
            <section>
              <button
                ...
                onClick={() => router.push("/weaver/ticketing")}
              >
                CANCEL
              </button>
            </section>
