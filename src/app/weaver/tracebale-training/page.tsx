...
const tracebaleTraining: React.FC = () => {
  ...
  }, [searchQuery, page, limit, isClear, weaverId]);

  const getTrainingList = async () => {
    const code = encodeURIComponent(searchQuery);
    const url = `training/training-process-status?weaverId=${weaverId}&limit=${limit}&page=${page}&search=${code}&mode=${checkedTrainingMode}&date=${from}&pagination=true`;
    try {
    ...
    if (name === "trainingMode") {
      if (checkedTrainingMode.includes(itemId)) {
        setCheckedTrainingMode(
          checkedTrainingMode.filter((item: any) => item !== itemId)
        );
      } else {
      ...
      cell: (row: any) => (
        <>
          ...
          {row.status !== "Pending" && (
            <>
              {row.status === "Accepted" ? (
                <button className="bg-green-500 text-white p-2 rounded hover:cursor-default">
                  Accepted
                </button>
              ) : (
                <button className="bg-red-500 text-white p-2 rounded hover:cursor-default">
                  Rejected
                </button>
              )}
