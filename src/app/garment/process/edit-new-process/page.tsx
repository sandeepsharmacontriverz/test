                                                ...
                                                <div className="row">
                                                    <div className="col-6 col-sm-6 mt-2">
                                                        ...
                                                        >
                                                            ...
                                                            {garmenttype.map((option: any, i: any) => (
                                                                <option key={i} value={option.name}>{option.name}</option>
                                                            ))}
                                                        </select> */}
                                                        <Select
                                                            name={`garmentType${index}`}
                                                            value={otherData.garmentType && otherData.garmentType.length > 0 ?
                                                                otherData.garmentType.map((id: any) => {
                                                                    const selectedOption = garmenttype.find((garment: any) => garment.id === parseInt(id, 10));
                                                                    return selectedOption ? { label: selectedOption.name, value: selectedOption.id } : null;
                                                                }) : null}
                                                ...
                                                <div className="row">
                                                    <div className="col-12 col-sm-6 mt-4">
                                                        ...
                                                        {/* <select
                                                            ...
                                                            onChange={(event) => handleChangeAdd(event, index)}
                                                        >
                                                            <option value="">Select Style Mark</option>
                                                            {styleMark.map((option: any, i: any) => (
                                                                <option key={i} value={option.style_mark_no}>{option.style_mark_no}</option>
                                                            ))}
                                                        </select> */}
                                                        <Select
                                                            name={`styleMarkNo${index}`}
                                                            value={otherData.styleMarkNo && otherData.styleMarkNo.length > 0 ?
                                                                otherData.styleMarkNo.map((id: any) => {
                                                                    const selectedOption = garmenttype.find((garment: any) => garment.id === parseInt(id, 10));
                                                                    return selectedOption ? { label: selectedOption.name, value: selectedOption.id } : null;
                                                                }) : null}
                                                            menuShouldScrollIntoView={false}
