...
const program: React.FC = () => {
  ...
  const getPrograms = async () => {
    const url = `program?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`
    try {
  ...
  const editHandle = (row: any) => {
    setEditId(row.id)
    setEditDefault(row.program_name)
    setEditVisible(true)
    ...
    if (data.length > 0) {
      const dataToExport = data.map((element: any, index: number) => {
        return {
          ...
          program_status: element.program_status
        }
      });
      exportToExcel(dataToExport, "Master-Program Data");
    } else {
  ...
  const changeStatus = async (row: any) => {
    const newStatus = !row.program_status;
    const url = "program/status"
    try {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row: any) => row.program_name,
      sortable: false,
    },
    {
      name: translations.common.status,
      cell: (row: any) => (
        <button onClick={() => changeStatus(row)} className={row.program_status ? "text-green-500" : "text-red-500"}>
          {row.program_status ? (
      ...
      <div >
        {isClient ? (
          <div >
                  ...
                  <div className="table-minwidth min-w-[650px]">
                    ...
                    <div className="search-filter-row">
                      ...
                      <div className="search-filter-right">
                        <button className="btn btn-all btn-purple" onClick={() => router.push("/master/program/add-program")} >{translations.common.add}</button>
                      </div>
                    </div>
            ...
            <div>
              <Edit openPopup={editVisible} onCancel={handleCancel} editId={editId} defaultValue={editDefault} getItems={getPrograms} />
            </div>
    ...
    if (!valid || !programs) {
      ...
      try {
        ...
        if (response.success) {
          ...
          toasterError(response.error.code === 'ALREADY_EXITS' ? 'Program Name already exist' : response.error.code);
          onCancel()
        }
      }
      catch (error) {
          ...
          <div className="bg-white border w-auto py-3 px-5 border-gray-300 shadow-lg rounded-md">
            ...
            <div className="py-2">
              ...
              <div className="flex py-3 justify-between">
                <span className="text-sm mr-8">Programs Name * </span>
                <div>
                  <input
                    type="text"
                    id="programs"
