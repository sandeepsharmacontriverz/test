...
export default function Page() {
  ...
  useTitle("Add Production Capacity")

  const numInputs = 16;
  const placeholderText = "Production Capacity";
  ...
  const addData = async () => {
    const url = "production-capacity/multiple"
    ...
    try {
      ...
      if (response.data.pass.length > 0) {
        ...
        toasterSuccess(`Following production capacity(s) have been added successfully: ${passedName} `)
      }
      if (response.data.fail.length > 0) {
        ...
        toasterError(`Following capacity(s) are skipped as they are already exist: ${failedName} `)
      }
      router.push('/master/production-capacity')
    }
    catch (error) {
  ...
  const handleSubmit = (e: any) => {
    ...
    inputValues.map((inputValue, index) => {
      ...
      if (index === 0 && inputValue === '') {
        newError[index] = 'Production Capacity is required';
        isError = true;
      } else if (!valid) {
        if (inputValue != '') {
        ...
        <div className="breadcrumb-inner light-bg">
          <div className="breadcrumb-left">
            <ul className="breadcrum-list-wrap">
              ...
              <li>Master</li>
              <li>
                <Link href="/master/production-capacity">
                  Production Capacity
                </Link>
              </li>
              <li>Add Production Capacity</li>
            </ul>
          </div>
        </div>
      </div>
            ...
            {inputValues.map((value, index) => (
              <div key={index} className="mb-3 mx-3">
                <input
                  ...
                  type="text"
                  placeholder={
                    index === 0 ? translations.productionCapacity.productionCapacityName + "*" : translations.productionCapacity.productionCapacityName
                  }
                  value={value}
