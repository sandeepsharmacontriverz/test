...
export default function Page() {
  ...
  const addData = async () => {
    const url = "cottonmix/multiple"
    const inputData = inputValues.filter((inputValue) => {
      return inputValue != '' || null;
    });
    try {
      const response = await API.post(url, {
        "cottonMixName": inputData
      })
      if (response.data.pass.length > 0) {
        const dataPassed = response.data.pass;
        const passedName = dataPassed.map((name: any) => name.data.cottonMix_name).join(', ')
        toasterSuccess(`Following cotton Mix name(s) have been added successfully: ${passedName} `)
      }
      if (response.data.fail.length > 0) {
        const dataFailed = response.data.fail;
        const failedName = dataFailed.map((name: any) => name.data.cottonMix_name).join(', ')
        toasterError(`Following cotton Mix name(s) are skipped as they are already exist: ${failedName} `)
      }
      router.push('/master/cotton-mix')
    }
    catch (error) {
  ...
  const handleSubmit = (e: any) => {
    ...
    inputValues.map((inputValue, index) => {
      ...
      if (index === 0 && inputValue === '') {
        newError[index] = 'Blend Name is required';
        isError = true;
      } else if (!valid) {
        if (inputValue != '') {
        ...
        <div className="breadcrumb-inner light-bg">
          <div className="breadcrumb-left">
            <ul className="breadcrum-list-wrap">
              ...
              <li>Master</li>
              <li>
                <Link href="/master/cotton-mix">
                  Cotton Mix
                </Link>
              </li>
              <li>Add Cotton Mix</li>
            </ul>
          </div>
        </div>
      </div>
            ...
            {inputValues.map((value, index) => (
              <div key={index} className="mb-3 mx-3">
                <input
                  ...
                  type="text"
                  placeholder={
                    index === 0 ? translations.cottonMix.blendName + "*" : translations.cottonMix.blendName
                  }
                  value={value}
