...
const SpinnerStockReport: any = () => {
    useTitle("Spinner Lint Cotton Stock Report");
    ...
    const getReports = async () => {
        const url = `reports/get-spinner-cotton-stock-report?countryId=${checkedCountries}&spinnerId=${checkedSpinners}&programId=${checkedPrograms}&seasonId=${checkedSeasons}&brandId=${brandId ? brandId : checkedBrands}&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`
        try {
    ...
    const handleExport = async () => {
        const url = `reports/export-spinner-cotton-stock-report?countryId=${checkedCountries}&spinnerId=${checkedSpinners}&programId=${checkedPrograms}&seasonId=${checkedSeasons}&brandId=${brandId ? brandId : checkedBrands}&status=&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`
        try {
            const response = await API.get(url)
            if (response.success) {
                if (response.data) {
                    handleDownload(response.data, 'Cotton Connect - Spinner Lint Cotton Stock Report', '.xlsx')
                }
            }
        }
        catch (error) {
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row: any) => row?.spinner_name,
            wrap: true,
        },
        {
        ...
        {
            name: <p className="text-[13px] font-medium">Total Lint Cotton Received (Kgs)</p>,
            selector: (row: any) => formatDecimal(row?.cotton_procured),
            wrap: true,
        }, {
            name: <p className="text-[13px] font-medium">Total Lint Cotton Consumed (Kgs) </p>,
            wrap: true,
            selector: (row: any) => formatDecimal(row?.cotton_consumed),
        }, {
...
export default SpinnerStockReport
