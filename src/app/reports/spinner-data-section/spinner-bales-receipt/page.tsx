...
const SpinnerBalesRecipt: any = () => {
  ...
  const handleExport = async () => {
    const url = `reports/export-spinner-bale-report?countryId=${checkedCountries}&ginnerId=${checkedGinners}&spinnerId=${checkedSpinners}&programId=${checkedPrograms}&seasonId=${checkedSeasons}&brandId=${brandId ? brandId : checkedBrands}&status=&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`
    try {
      const response = await API.get(url)
      if (response.success) {
        if (response.data) {
          handleDownload(response.data, 'Cotton Connect - Spinner Bale Reciept Report', '.xlsx')
        }
      }
    }
    catch (error) {
  ...
  const getReports = async () => {
    const url = `reports/get-spinner-bale-report?countryId=${checkedCountries}&ginnerId=${checkedGinners}&spinnerId=${checkedSpinners}&programId=${checkedPrograms}&seasonId=${checkedSeasons}&brandId=${brandId ? brandId : checkedBrands}&status=Sold&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`
    try {
      const response = await API.get(url)
  ...
  const handleFilterChange = (selectedList: any, selectedItem: any, name: string, remove: boolean = false) => {
    ...
    if (name === "countries") {
      ...
      if (checkedGinners?.includes(itemId)) {
        setCheckedGinners(checkedGinners?.filter((item: any) => item !== itemId));
      } else {
  ...
  const columns = [
          ...
          {row.quality_report && (
            <>
              <FaEye
                ...
                onClick={() => router.push(`/reports/spinner-data-section/spinner-bales-receipt/lot-test-quality?id=${row?.quality_report?.id}`)}
              />
              <FaDownload
                size={18}
                className="text-black ml-3 hover:text-blue-600 cursor-pointer"
    ...
    {
      name: <p className="text-[13px] font-medium"> {translations.ginnerInterface.lotNo}</p>,
      wrap: true,
