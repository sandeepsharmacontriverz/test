...
const SpinnerYarnProcess: any = () => {
  ...
  const handleExport = async () => {
    const url = `reports/export-spinner-yarn-report?brandId=${brandId ? brandId : checkedBrands}&programId=${checkedPrograms}&countryId=${checkedCountries}&spinnerId=${checkedSpinners}&seasonId=${checkedSeasons}&page=${page}&limit=${limit}&search=${searchQuery}&pagination=true`
    try {
      const response = await API.get(url)
      if (response.success) {
        if (response.data) {
          handleDownload(response.data, 'Cotton Connect - Spinner Yarn Process', '.xlsx')
        }
      }
    }
    catch (error) {
  ...
  const getReports = async () => {
    const url = `reports/get-spinner-yarn-report?brandId=${brandId ? brandId : checkedBrands}&countryId=${checkedCountries}&spinnerId=${checkedSpinners}&programId=${checkedPrograms}&seasonId=${checkedSeasons}&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`
    try {
      const response = await API.get(url)
      if (response.success) {
  ...
  const getBlendNames = (ids: any) => {
    const matchId = cottonMix?.filter((cottonMix: any) => ids?.includes(cottonMix.id))
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row: any) =>  row.yarncount?.map((item:any) => item.yarnCount_name)?.join(', '),
    }, {
      name: <p className="text-[13px] font-medium"> {translations.spinnerInterface.yarnRealistation} </p>,
    ...
    {
      name: <p className="text-[13px] font-medium"> {translations.spinnerInterface.blend} </p>,
      cell: (row: any) => getBlendNames(row?.cottonmix_type)
    },
    {
      name: <p className="text-[13px] font-medium"> {translations.spinnerInterface.blendQuantity} </p>,
      cell: (row: any) => row?.cottonmix_qty?.join(',')
    },
    {
      name: <p className="text-[13px] font-medium"> Total Seed cotton consumed (Kgs) </p>,
    ...
    {
      name: <p className="text-[13px] font-medium">Total Yarn in Stock (KGs)</p>,
