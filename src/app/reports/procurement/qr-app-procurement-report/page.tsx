...
const QrProcurementReport: any = () => {
  ...
  const getTransactions = async () => {
   try {
      const res = await API.get(
        `qr-app/get-qr-transactions?search=${debouncedSearch}&limit=${limit}&page=${page}&pagination=true&brandId=${brandId ? brandId : checkedBrand}&countryId=${checkedCountry}&programId=${checkedProgram}&ginnerId=${checkedGinner}&seasonId=${checkedSeason}&agentId=${checkedAgent}&startDate=${
 ...
 const formatDecimal = (value: string | number): string | number => {
  ...
  const fetchExportAll = async () => {
    try {
      const res = await API.get(
        `qr-app/export-qr-transactions?search=${debouncedSearch}&limit=${limit}&page=${page}&pagination=true&brandId=${brandId ? brandId :checkedBrand}&countryId=${checkedCountry}&programId=${checkedProgram}&ginnerId=${checkedGinner}&seasonId=${checkedSeason}&agentId=${checkedAgent}&startDate=${
          from ? from?.toISOString() : ""
        }&endDate=${to ? to?.toISOString() : ""}`
      );
      if (res.success) {
