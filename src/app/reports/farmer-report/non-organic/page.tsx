...
const NonOrganic: any = () => {
    ...
    const fetchNonOrganic = async () => {
        try {
            const res = await API.get(`reports/get-farmer-report?type=non-Organic&search=${code}&brandId=${checkedBrand}&farmGroupId=${checkedFarmGroup}&icsId=${checkedIcsName}&limit=${limit}&countryId=${checkedCountries}&stateId=${checkedStates}&districtId=${checkedDistricts}&blockId=${checkedBlocks}&villageId=${checkedVillages}&page=${page}&pagination=true`);
            if (res.success) {
                setData(res.data)
                setCount(res.count)
            }
    ...
    const fetchExportAll = async () => {
        const url = `reports/export-non-farmer-report?countryId=${checkedExportCountries}&stateId=${checkedExportStates}&districtId=${checkedExportDistricts}&blockId=${checkedExportBlocks}&villageId=${checkedExportVillages}&brandId=${checkedExportBrands}&search=${code}&page=${page}&limit=${limit}&pagination=true`;
        ...
        try {
            ...
            if (response.success) {
                if (response.data) {
                    handleDownload(response.data, "CottonConnect - Non Organic Report", ".xlsx");
                    exportClear();
    ...
    const columns = [
        ...
        {
            name: (<p className="text-[13px] font-medium"> {translations.common.cottonArea}</p>),
