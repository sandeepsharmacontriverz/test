...
const WeaverFabricSale: any = () => {
  const [roleLoading] = useRole();
  useTitle("Weaver Fabric Sales Report");
  ...
  const handleExport = async () => {
    const url = `reports/export-weaver-sales-report?countryId=${checkedCountries}&brandId=${brandId ? brandId : checkedBrands}&seasonId=${checkedSeasons}&programId=${checkedPrograms}&spinnerId=${checkedWeavers}&fabricType=${checkedFabric}&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`;
    try {
      const response = await API.get(url);
      if (response.success) {
        if (response.data) {
          handleDownload(response.data, "Cotton Connect - Weaver Fabric Sale Report", ".xlsx");
        }
      }
    } catch (error) {
  ...
  const getReports = async () => {
    const url = `reports/get-weaver-sales-report?countryId=${checkedCountries}&brandId=${brandId ? brandId : checkedBrands}&seasonId=${checkedSeasons}&programId=${checkedPrograms}&spinnerId=${checkedWeavers}&fabricType=${checkedFabric}&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`;
    try {
      const response = await API.get(url);
  ...
  const columns = [
    ...
    // },
    {
      name: <p className="text-[13px] font-medium">Finished Lot No </p>,
    ...
    },
    {
      ...
      selector: (row: any) => formatDecimal(row?.net_length),
      wrap: true,
    },
    {
...
export default WeaverFabricSale;
