...
const StockReport: any = () => {
    ...
    const getReports = async () => {
        const url = `reports/get-ginner-cotton-stock-report?ginnerId=${checkedGinners}&countryId=${checkedCountries}&programId=${checkedPrograms}&seasonId=${checkedSeasons}&brandId=${brandId ? brandId : checkedBrands}&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`
        try {
    ...
    const handleExport = async () => {
        const url = `reports/export-ginner-cotton-stock-report?ginnerId=${checkedGinners}&programId=${checkedPrograms}&seasonId=${checkedSeasons}&brandId=${brandId ? brandId : checkedBrands}&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`
        try {
            const response = await API.get(url)
            if (response.success) {
                if (response.data) {
                    handleDownload(response.data, 'Cotton Connect - Ginner Seed Cotton Stock Report', '.xlsx')
                }
            }
        }
        catch (error) {
    ...
    const columns = [
        ...
        {
            name: <p className="text-[13px] font-medium">Total Seed Cotton Procured (Kgs)</p>,
            selector: (row: any) => formatDecimal(row?.cotton_procured),
            wrap: true,
        }, {
            name: <p className="text-[13px] font-medium">Total Seed Cotton in Processed (Kgs) </p>,
            wrap: true,
            selector: (row: any) => formatDecimal(row?.cotton_processed)
        }, {
            name: <p className="text-[13px] font-medium">Total Seed Cotton in Stock (Kgs)</p>,
            wrap: true,
