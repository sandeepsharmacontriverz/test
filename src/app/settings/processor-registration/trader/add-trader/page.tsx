...
export default function page() {
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (
      ...
      try {
        const response = await API.post("trader", {
          ...processorFormData,
          userData: users
        });
        if (response.success) {
          toasterSuccess("Trader created Successfully");
  ...
  const alreadyExistName = async (name: string, value: string, index: number) => {
    const res = await API.post("trader/check-trader", {
      [name]: value
    });
      ...
      <div>
        ...
        <div className="justify-between mt-4 px-2 space-x-3 ">
          ...
          <button
            ...
            onClick={() => router.push('/settings/processor-registration/trader')}
          >
            Cancel
          </button>
        </div>
