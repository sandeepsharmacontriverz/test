...
export default function page() {
  ...
  const fetchGinners = async () => {
    try {
      const res = await API.get(
        `ginner?countryId=${checkedCountries}&stateId=${checkedStates}&limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`
      );
      if (res.success) {
        setData(res.data);
        setCount(res.count);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row: any) => (
        <Link
          legacyBehavior
          href={`/settings/processor-registration/ginner/view-ginner?id=${row.id}`}
          passHref
        >
  ...
  const filterData = async () => {
    try {
      const res = await API.get(
        `ginner?countryId=${checkedCountries}&stateId=${checkedStates}&limit=${limit}&page=${page}&search=${searchFilter}&pagination=true`
      );
      if (res.success) {
        setData(res.data);
        setCount(res.count);
        ...
        <div className="breadcrumb-inner light-bg">
          <div className="breadcrumb-left">
            <ul className="breadcrum-list-wrap">
              ...
              <li>Settings</li>
              <li>
                <Link href="/services/scope-certification">Ginner</Link>
              </li>
            </ul>
          </div>
        </div>
