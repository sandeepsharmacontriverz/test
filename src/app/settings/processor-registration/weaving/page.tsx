...
export default function page() {
  ...
  const getWeaverList = async () => {
    const url = `weaver?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`;
    try {
  ...
  const filterData = async () => {
    try {
      const res = await API.get(
        `weaver?search=${searchFilter}&countryId=${checkedCountries}&stateId=${checkedStates}&limit=${limit}&page=${page}&pagination=true`
      );
      if (res.success) {
        setData(res.data);
        setCount(res.count);
  ...
  const columns = [
    ...
    {
      name: "Weaving Unit Name",
      selector: (row: any) => row.name,
      cell: (row: any) => (
        <Link
          legacyBehavior
          href={`/settings/processor-registration/weaving/view-weaving?id=${row.id}`}
          passHref
        >
              ...
              <div className="search-filter-row">
                ...
                <div className="search-filter-right">
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        "/settings/processor-registration/weaving/add-weaving"
                      )
                    }
                  >
                    Add
