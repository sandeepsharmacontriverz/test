...
export default function page() {
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (
      ...
      try {
        const response = await API.post("weaver", {
          ...processorFormData,
          loomType: String(processorFormData.loomType),
          userData: users,
        });
        if (response.success) {
          toasterSuccess("Weaver Successfully Created");
  ...
  const alreadyExistName = async (name: string, value: string, index: number) => {
    const res = await API.post("weaver/check-weaver", {
      [name]: value
    });
      ...
      <div className="bg-white rounded-md p-4">
        ...
        <div className="justify-between mt-4 px-2 space-x-3 ">
          ...
          <button
            ...
            onClick={() => router.push('/settings/processor-registration/weaving')}
          >
            Cancel
          </button>
        </div>
