...
export default function page() {
    ...
    useEffect(() => {
        if (selectedId) getSpinner();
    }, [selectedId]);

    const getSpinner = async () => {
        const url = `spinner/get-spinner?id=${selectedId}`
        try {
            ...
            setProcessorFormData(
                {
                    ...
                    rangeFrom: prevData.realisation_range_from,
                    rangeTo: prevData.realisation_range_to,
                    yarnCountRange: prevData.yarn_count_range,
                    yarnType: prevData.yarn_type,
                }
            );
    ...
    const handleSubmit = async (event: any) => {
        ...
        if (
            !hasSpinnerErrors && !hasUserErrors
        ) {
            try {
                const response = await API.put("spinner", {
                    ...processorFormData,
