...
export default function page() {
  ...
  const getGarmentList = async () => {
    const url = `garment?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`;
    try {
  ...
  const filterData = async () => {
    try {
      const res = await API.get(
        `garment?search=${searchFilter}&countryId=${checkedCountries}&stateId=${checkedStates}&limit=${limit}&page=${page}&pagination=true`
      );
      if (res.success) {
        setData(res.data);
        setCount(res.count);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row: any) => (
        <Link
          legacyBehavior
          href={`/settings/processor-registration/garment/view-garment?id=${row.id}`}
          passHref
        >
