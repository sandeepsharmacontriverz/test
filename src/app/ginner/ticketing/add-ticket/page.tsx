...
export default function Page() {
  ...
  useEffect(() => {
    if (ginnerId && formData.dataCorrectionType === "Sale") {
      fetchSales();
      fetchTicketType("sales");
    } else if (ginnerId && formData.dataCorrectionType === "Process") {
      fetchProcesses();
      fetchTicketType("process");
    }
  }, [ginnerId, formData.dataCorrectionType]);
  ...
  const fetchSales = async () => {
    try {
      const response = await API.get(
        `ginner-process/sales?ginnerId=${ginnerId}`
      );
      if (response.success) {
  ...
  const fetchTicketType = async (type: any) => {
    try {
      const response = await API.get(
        `ticketing/escalation-type?processorType=ginner&correctionType=${type}`
      );
      if (response.success) {
          ...
          <div className="breadcrumb-inner light-bg">
            <div className="breadcrumb-left">
              <ul className="breadcrum-list-wrap">
                ...
                <li>
                  <NavLink href="/ginner/ticketing">Ticketing</NavLink>
                </li>
                ...
                <div className="row">
                  ...
                  <div className="col-sm-12 col-lg-6 col-md-6 mt-4">
                    ...
                    <Select
                      ...
                      className="dropDownFixes rounded-md formDropDown mt-1 text-sm borderBottom"
                      options={(formData.dataCorrectionType === "Process" ? processlot : salesLot || []).map(({ id, lot_no }: any) => ({
                        label: lot_no,
                        value: id,
                        key: id
                      }))}
          ...
          <div className="pt-10 w-100 d-flex justify-start customButtonGroup pb-5">
            <section>
              <button
                ...
                onClick={() => router.push("/ginner/ticketing")}
              >
                CANCEL
              </button>
            </section>
